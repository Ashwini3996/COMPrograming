...
HRESULT CSumSubtract::InitializeInnerComponent(void)
{
	...
	if (FAILED(hr))
	{
		MessageBox(NULL, TEXT("IMultiplication Interface Can Not Be Obtained From Inner Commponet."),
			TEXT("Error"), MB_OK);
		return(E_FAIL);
	}
	...
	if (FAILED(hr))
	{
		MessageBox(NULL, TEXT("IDivision Interface Can Not Be Obtained From Inner Commponet."),
			TEXT("Error"), MB_OK);
		return(E_FAIL);
	}
	return(S_OK);
